...
## 2026-09-25：全量结果与来源排序细化

用户已上传完整source-first汇总：17,790答/2,903,263 token；test为2,700答/424,408 token。
开发选择Data2txt/QA的pair来源均值、Summary的full均值，test AUROC=.784388/.878533/.759659；
此前“尚无全量自然成绩”已被更新，但新native中介仍无真实8B成绩。
QA窗口route单元内AUROC=.831193，Data2txt/Summary=.560711/.555733，不能统一套用QA机制。
`main.py transport-refine`复用source-first原始标量，协议见`iclr/RAGTRUTH_TOKEN_REFINEMENT.md`。
三种来源锚点×token来源/窗口来源/窗口route，比较严格同分细化和小幅残差；保留所有基线。
整数锚点块保留严格原排序，不保证AUROC改善；同分分解包含不同单元恰好同分的token对。
评分先冻结，再只用原20% train开发来源选择每task的方法；同task全部test生成器统一应用。
