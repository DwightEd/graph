...
from route_graph.archive import FeatureArchive, atomic_json, digest
...
def calibrate(archive: Path, output: Path, quantile=0.95):
    ...
    atomic_json(output / "thresholds.json", result)
    return result
...
def evaluate(
    ...
    # Schema @1 calibration fixes k=3 and cap=8; the scorer also allows other settings.
    if capture.get("neighbors") != 3 or capture.get("per_source") != 8:
        raise ValueError(
            "calibration requires scoring with neighbors=3 and per_source=8"
        )
