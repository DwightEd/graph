...
class ResearchSample:
    ...
    def node_features(self, mode="attention", attention=None):
        """Load node features, optionally reusing an already-loaded attention sample."""
    ...
    def graph(self, name):
        ...
        graph = torch.load(
            path, map_location=self.dataset.device, weights_only=True
        )
    ...
    def attention_edges(self, attention=None):
        ...
        Pass an already-loaded attention sample to avoid reading the same NPZ twice
        in analyses that need both attention metadata and decoded edges.
        """
