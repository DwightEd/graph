"""Label-free exact-input export of preexisting observer baseline measurements."""
import argparse
import json
...
def main():
    ...
    rows=[r for r in map(json.loads,Path(args.inputs).read_text().splitlines()) if r['split']==args.phase]
    validate_rows(rows)
    assert len(rows)==32
