"""Same frozen64 metrics with fail-closed ID-first annotation parsing; no fitting."""
import argparse
import hashlib
import json
import re
...
ID_PREFIX = re.compile(rb'^\s*\{\s*"id"\s*:\s*(?:"([0-9]+)"|([0-9]+))\s*[,}]')


def selected_annotations(path, wanted):
    """RAGTruth's pinned ID-first JSONL schema; never decode unrelated labels.

    Fail closed for a different schema rather than fall back to parsing all rows.
    The anchored prefix cannot mistake an escaped ID in response text for the ID.
    """
    gt = {}
    with Path(path).open('rb') as stream:
        for line_number, raw in enumerate(stream, 1):
            ...
            if match is None:
                raise ValueError(f'unsupported annotation ID-first schema at line {line_number}')
            rid = (match.group(1) or match.group(2)).decode('ascii')
            ...
            if str(record['id']) != rid:
                raise ValueError('annotation ID-prefix mismatch')
            gt[rid] = record
    if set(gt) != wanted:
...
def check_scope_addendum(path):
    note = json.loads(Path(path).read_text())
    graph = Path(__file__).resolve().parents[1]
    freeze = graph/'outputs/P6_CONFIRMATION_FREEZE_20260914.json'
    if note.get('parent_freeze_sha256') != 'ffe0607ed0be030310d12c32c4145de6e13d56a12d7d381f2d68b1c09df307d5':
        raise ValueError('unexpected parent freeze')
    if sha(freeze) != note['parent_freeze_sha256'] or sha(__file__) != note.get('scoped_evaluator_sha256'):
        raise ValueError('scope addendum identity mismatch')
    if sha(Path(__file__).with_name('confirmation_assess.py')) != note.get('original_evaluator_sha256'):
        raise ValueError('original evaluator was changed')
    if note.get('change_scope') != 'annotation_ID_first_parsing_only_no_metric_or_score_change':
        raise ValueError('unsupported evaluation change')
...
def main():
    ...
    p.add_argument('--primary', default='reviewed_source_risk')
    p.add_argument('--reference-predictions', action='append', default=[])
    p.add_argument('--bootstrap', type=int, default=500)
    p.add_argument('--scope-addendum', required=True)
    args = p.parse_args()
    if Path(args.output).exists():
        raise FileExistsError(args.output)
    check_scope_addendum(args.scope_addendum)
    m, records = frozen_records(args.predictions)
    if m.get('input_sha256') != 'bc9fabcad838541e8f8892d436a181746c7b200363b9160b73b50ce2291d5101':
        raise ValueError('not fixed confirmation inputs')
    if m['settings']['phase'] != 'validation' or len(records) != 64 or len({str(r['source_id']) for r in records}) != 32:
        raise ValueError('incomplete fixed64 confirmation denominator')
    if m['settings']['phase'] == 'pilot':
        raise ValueError('pilot may not join natural annotations')
    reference_hashes = {}
    ...
    report = dict(phase=m['settings']['phase'], retrospective=True, primary_score=args.primary,
        source_scope='32 fixed official-test sources disjoint from R04; prior population measurements exist, not globally pristine',
