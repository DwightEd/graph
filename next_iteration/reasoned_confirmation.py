"""P7 frozen new128 confirmation adapter; identical reasoned scoring, no GT."""
import argparse
import hashlib
import json
import re
...
PARENT_CODE_SHA = 'f6bb3332d3743733d5d8337e6753978c4ff7ded4a2d6476121bfd9f10a54589f'

THINK_BUDGET = 1024
FINAL_BUDGET = 384
SAMPLING = dict(do_sample=True, temperature=0.6, top_p=0.95, top_k=20, min_p=0.0)


def audit_seed(source, instruction, response):
    payload = json.dumps([source, instruction, response], ensure_ascii=False).encode()
    return (20260914 + int.from_bytes(hashlib.sha256(payload).digest()[:4], 'big')) % (2**31)


def final_context(tokenizer, prompt_ids, reasoning_ids, eos_ids, close_id):
    """Retain raw generation separately; explicit budget closure is never model output."""
    natural_close = bool(reasoning_ids) and reasoning_ids[-1] == close_id
    content = list(reasoning_ids)
    if natural_close:
        tail = tokenizer.encode('\n\n', add_special_tokens=False)
    else:
        while content and content[-1] in eos_ids:
            content.pop()
        tail = tokenizer.encode(
            '\n\nThe reasoning budget is reached. Give the concise source-grounded audit now.\n</think>\n\n',
            add_special_tokens=False)
    return list(prompt_ids) + content + tail, dict(natural_think_close=natural_close,
        forced_close=not natural_close, appended_context_ids=tail,
        retained_reasoning_ids=content, raw_reasoning_ids=list(reasoning_ids))
...
def main():
    ...
    from .reasoned_confirmation_freeze_check import check
    root=Path(__file__).resolve().parents[1]
    freeze_path=root/'outputs/P7_CONFIRMATION_FREEZE_20260914.json'
    freeze=check(freeze_path)
    if digest(Path(__file__).with_name('local_grounding_reasoned.py')) != PARENT_CODE_SHA:
        raise ValueError('parent P7 implementation changed')
    if args.batch_size != 4 or digest(args.inputs) != INPUT_SHA:
        raise ValueError('invalid batch size or input SHA')
    expected_model = Path(__file__).resolve().parents[3] / 'models/Qwen3-8B'
    if Path(args.model).resolve() != expected_model.resolve():
        raise ValueError('not the frozen local model directory')
    rows = [json.loads(s) for s in Path(args.inputs).read_text().splitlines()]
    validate_rows(rows)
    if len(rows) != 128 or len({str(r['source_id']) for r in rows}) != 64:
        raise ValueError('fixed128 confirmation denominator changed')
    if any(r['split'] != 'validation' or r['official_split'] != 'test' for r in rows):
        raise ValueError('not fixed official-test128 inputs')
    out = Path(args.output)
    if out.resolve()!=(root/freeze['output']).resolve():
        raise ValueError('output differs from frozen single endpoint')
    ...
    manifest = dict(settings=vars(args), input_sha256=INPUT_SHA, code_sha256=digest(__file__),
                    ...
                    started_unix=started, primary_score='reasoned_source_risk',
    ...
    manifest['confirmation_freeze_sha256'] = digest(freeze_path)
    manifest['confirmation_freeze_created_utc'] = freeze['created_utc']
    manifest['source_scope'] = '64 fixed official-test sources disjoint from R04 and spentP6; prior population measurements exist, not globally pristine'
    manifest['audit_max_new_tokens'] = FINAL_BUDGET
    manifest['reasoning_budget'] = THINK_BUDGET
    manifest['draft_sampling'] = SAMPLING
    manifest['seed_rule'] = '20260914 + SHA256([source,instruction,response]) first4 bytes modulo2^31; one sample, no best-of-K'
    manifest['change_from_P6'] = 'audit generation only: bounded thinking then final audit; same auditor instructions, same word query, precision, raw score and mapping'
    protocol = Path(__file__).resolve().parents[1] / 'docs/P7_REASONED_AUDIT_PROTOCOL.md'
    manifest['protocol_sha256'] = digest(protocol)
    (out / 'executed_protocol.md').write_bytes(protocol.read_bytes())
    ...
    try:
        ...
        def draft_audit(source, instruction, response):
            ids = tokenizer.apply_chat_template(audit_messages(source, instruction, response),
                tokenize=True, add_generation_prompt=True, enable_thinking=True)
            if len(ids) + THINK_BUDGET + FINAL_BUDGET + 64 > model.config.max_position_embeddings:
            ...
            if len(closing) != 1:
                raise ValueError('thinking close token must be atomic')
            seed = audit_seed(source, instruction, response)
            torch.manual_seed(seed)
            x = torch.tensor([ids], device='cuda')
            reasoning = model.generate(input_ids=x, attention_mask=torch.ones_like(x),
                max_new_tokens=THINK_BUDGET, eos_token_id=list(set(eos + closing)),
                use_cache=True, pad_token_id=pad, **SAMPLING)
            reasoning_ids = reasoning[0, len(ids):].cpu().tolist()
            final_ids, closure = final_context(tokenizer, ids, reasoning_ids, eos, closing[0])
            if len(final_ids) + FINAL_BUDGET > model.config.max_position_embeddings:
                raise ValueError('final audit context overflow; truncation forbidden')
            del reasoning, x
            x = torch.tensor([final_ids], device='cuda')
            final = model.generate(input_ids=x, attention_mask=torch.ones_like(x),
                max_new_tokens=FINAL_BUDGET, eos_token_id=eos,
                use_cache=True, pad_token_id=pad, **SAMPLING)
            new_ids = final[0, len(final_ids):].cpu().tolist()
            truncated = not new_ids or new_ids[-1] not in eos
            return dict(text=tokenizer.decode(new_ids, skip_special_tokens=True),
                generated_ids=new_ids, input_tokens=len(ids), final_input_tokens=len(final_ids),
                truncated=truncated, seed=seed, reasoning_generated_ids=reasoning_ids,
                reasoning_text=tokenizer.decode(reasoning_ids, skip_special_tokens=False),
                reasoning_budget_exhausted=len(reasoning_ids)==THINK_BUDGET and reasoning_ids[-1]!=closing[0],
                closure=closure)
        ...
        for row in rows:
            ...
            print(json.dumps(dict(id=row['id'], reasoning_tokens=len(draft['reasoning_generated_ids']),
                                  reasoning_forced_close=draft['closure']['forced_close'],
                                  draft_tokens=len(draft['generated_ids']),
            ...
            result = dict(id=row['id'], source_id=row['source_id'], task=row['task'], split=row['split'],
                          ...
                          prompt_sha256=row['prompt_sha256'], scores={'reasoned_source_risk': scores},
        ...
        manifest.update(complete=True, elapsed_seconds=time.time() - started,
            ...
            engineering_artifact_sha256={p.name: digest(p) for p in [out/'canaries.json',out/'executed_protocol.md']},
