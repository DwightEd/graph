"""P5: frozen full-context word grounding. Retrospective, no annotation input."""
import argparse
import hashlib
import json
import re
...
def main():
    ...
    manifest = dict(settings=vars(args), input_sha256=INPUT_SHA, code_sha256=digest(__file__),
                    ...
                    started_unix=started, primary_score='localized_source_risk',
    ...
    try:
        ...
        for source, response, target, expected, name in CANARIES:
            ...
            vals, audit = verify([messages(source, 'Report only source-supported facts.', response,
                                          (start, start + len(target)))])
            canaries.append(dict(name=name, source=source, response=response, target=target,
                                 expected=expected, result=vals[0], cache_audit=audit))
        ...
        for row in rows:
            ...
            vals, cache_audit = verify([messages(source, instruction, row['response'], span) for span in spans])
            ...
            result = dict(id=row['id'], source_id=row['source_id'], task=row['task'], split=row['split'],
                          ...
                          prompt_sha256=row['prompt_sha256'], scores={'localized_source_risk': scores},
