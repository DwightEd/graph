"""P7 new128 official-test answers; excludes R04 and spent P6, no labels/scores."""
import argparse
import hashlib
import json
...
QUOTAS={'QA':24,'Summary':24,'Data2txt':16}
P6_INPUT_SHA='bc9fabcad838541e8f8892d436a181746c7b200363b9160b73b50ce2291d5101'
...
def main():
    ...
    p.add_argument('--spent-p6',required=True)
    ...
    if digest(args.spent_p6)!=P6_INPUT_SHA:raise ValueError('spent P6 identity')
    old=list(map(json.loads,Path(args.r04).read_text().splitlines()))
    old += list(map(json.loads,Path(args.spent_p6).read_text().splitlines()))
    validate_rows(old)
    ...
    for line in Path(args.population_inputs).open():
        ...
        if sid not in grouped:
            grouped[sid]={'source_id':sid,'source_sha256':s_sha,'task':r['task'],
                          'select_sha256':text_sha(f'P7-confirm-v1|20260914|{sid}|{s_sha}'),'responses':{}}
        g=grouped[sid]
    ...
    for g in selected:
        for generator in GENERATORS:
            ...
            rows.append(r)
    validate_rows(rows)
    if missing or len(rows)!=128 or len(selected)!=64:
        raise ValueError('complete fixed128 roster required; never replace missing-generator sources')
    ...
    manifest=dict(status='complete',settings=vars(args),protocol='P7-confirm-v1|20260914; SHA order; source+exact evidence dedup',
        code_sha256=digest(__file__),r04_inputs_sha256=INPUT_SHA,population_inputs_sha256=POPULATION_SHA,
        spent_p6_inputs_sha256=P6_INPUT_SHA,
        input_sha256=digest(out/'inputs.jsonl'),roster_sha256=digest(out/'roster.json'),
        planned_sources=64,planned_responses=128,sources=len(selected),responses=len(rows),missing=missing,
        by_task=QUOTAS,excluded_r04_and_p6_source_ids=sorted(excluded_ids),labels_joined=False,model_forwards=0,
        selection_does_not_use=['labels','quality','scores','response length','confidence','resource fit'],
        scope='official-test source holdout relative to R04 and spent P6; previous population measurements touched these sources; not globally unseen',
        confirmation_policy='all128 responses as single primary denominator; no partial-cohort peeking or sample replacement; roster selected before P7 pilot/development natural metrics')
