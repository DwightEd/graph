...
from next_iteration.grounded_graph_restoration_train import PROTOCOL as TRAIN_PROTOCOL
...
PROTOCOL = {"schema": "grounded-graph-restoration-natural-prediction@2", "labels_read": False,
    "score": "graph_difference = logp_full - logp_restore; restoration_difference = logp_empty - logp_restore; both high risk",
    "scores": ["base_nll", "base_entropy", "empty_nll", "graph_restoration_difference", "no_edges_restoration_difference", "permuted_graph_restoration_difference", "graph_nll", "graph_difference", "no_edges_nll", "no_edges_difference", "permuted_graph_difference"],
...
def score_one(adapters, head, packet, arrays, seed):
    ...
    base_logp, base_entropy = token_statistics(torch.from_numpy(arrays["full_query"]).to(head.device), targets, head)
    empty_logp, _ = token_statistics(arguments["query_states"], targets, head)
    scores = {"base_nll": -base_logp.cpu().numpy(), "base_entropy": base_entropy.cpu().numpy(), "empty_nll": -empty_logp.cpu().numpy()}
    traces = {}
    for arm in TRAIN_PROTOCOL["arms"]:
        ...
        scores[arm + "_restoration_difference"] = (empty_logp - logp).cpu().numpy()
    ...
    scores["permuted_graph_restoration_difference"] = (empty_logp - logp).cpu().numpy()
...
def run(args):
    ...
    code = {**train_settings["code_sha256"], "next_iteration/grounded_graph_restoration_predict.py": file_sha256(Path(__file__))}
