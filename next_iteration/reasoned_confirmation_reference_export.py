"""Label-free fixed128 P7 export of unchanged observer baseline measurements."""
import argparse
import json
from pathlib import Path
import numpy as np
from .grounding_contrast import digest, validate_rows, write_json
from .reasoned_confirmation_freeze_check import check, ROOT
from .reasoned_confirmation_contract import (POPULATION, POPULATION_SETTINGS_SHA,
    POPULATION_COMPLETE_SHA, EXPECTED_POPULATION_CONTROLS)
...
def main():
    ...
    args=p.parse_args()
    freeze=check(ROOT/'outputs/P7_CONFIRMATION_FREEZE_20260914.json')
    ...
    if population.resolve()!=POPULATION.resolve():raise ValueError('not the pinned population root')
    if digest(population/'settings.json')!=POPULATION_SETTINGS_SHA or digest(population/'COMPLETE')!=POPULATION_COMPLETE_SHA:
    ...
    manifest=dict(settings=vars(args),input_sha256=INPUT_SHA,code_sha256=digest(__file__),
                  ...
                  parent_settings_sha256=digest(population/'settings.json'),
                  parent_complete_sha256=digest(population/'COMPLETE'),parent_manifests={})
    (out/'executed_code.py').write_bytes(Path(__file__).read_bytes())
    write_json(out/'manifest.json',manifest)
    for r in rows:
        root=population/'responses'/str(r['id'])
        if digest(root/'manifest.json')!=freeze['population']['parent_manifest_sha256'][str(r['id'])]:
            raise ValueError('frozen parent manifest identity')
        ...
        n=len(r['offsets'])
        if set(scores)!=EXPECTED_POPULATION_CONTROLS:raise ValueError('exact ten-control schema')
