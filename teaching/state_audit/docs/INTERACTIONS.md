...
- total_readout / total_write：选中头的实际总状态，包含 Inject/Replace 的修改。
- mass：指定来源的原始 attention 质量；整头模式未另外测来源质量，返回 None。

来源消息恢复通过 `Inject(head_readout, reference − current)` 实现，不重分配 attention。
因此恢复后 raw A·V 路线分量仍反映当前读取，实际加回内容体现在 total_readout/write。
