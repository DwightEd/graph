...
2026-09-13，结构修订4。初稿、四轮6.22/6.90/7.12/7.79分审查及历次完整方案保留。此文是待复审的方法，不是训练或检测结果。
...
两个对照固定H、λ、移出质量、query/layer，只把目的地改成已匹配的无关source组C。specificity=Δ(H→S)−max_C|Δ(H→C)|，同一个F、正方向表示将H分配给S特别降低原事件偏好。负方向也保存，但不叫history-over-source错误的支持。broad R始终是allocation_effect_only；paired R通过才叫selective_cross_role_effect。表面语义与输入介导不充分时保留position前缀，不能升级为错误内容的路由。
...
主线模块放graph：evidence_anchor.py负责不泄漏的语义输入与图锚点；causal_contrast.py负责有限事件与E/V量；causal_groups.py负责预算搜索；grounded_audit.py负责覆盖、分组和输出。graph承载可复用原生oracle，reanchor仅调用作机制执行，避免主线依赖机制脚本。当前causal_contrast.py有限事件/E-V/paired-transfer内核、native_audit.py真实Llama E/V/R/M与branch-matched donor重放后端已实现；语义锚点、native自动搜索、证书编排与批量检测入口尚未实现；旧算子在ownership/adoption可复用，但不能把旧实现声称完整新方法。首版没有训练脚本。
