...
原始后验节点定位任务不变。
## Simplicity Check
只修裁决一致性、同来源成对统计及hidden身份，不增加检测模块。内存上限只约束属性，不冒称RSS限制。
...
所有通道中严格大于 floor 的边保留并按通道总数平均；不对稀疏保留质量重归一化，不建立 N×N 中间数组。上游未保存质量单列 unknown。正式CSR只保存response query的边；prompt节点属性存在，但prompt→prompt边未观测。本版保留完整节点集合及缓存中的可用响应边，不能称为已捕获完整逐层属性图。层平均边只是 token 依赖代理，不是跨 Transformer 层的 attention rollout。
