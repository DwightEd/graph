...
## Simplicity Check
工程阻塞已解除；不为提高评审分数添加模块或将合成结果改称自然证据。第三/四轮未提出额外可修复代码项，保留自然评价门槛。
...
所有通道中严格大于 floor 的边保留并按通道总数平均；不对小于1的稀疏保留质量重归一化，不建立 N×N 中间数组。仅当FP16舍入导致保留质量+self略大于1（单通道容忍至1.002）时，按该行总量修正数值溢出；真正丢弃的质量仍保持unknown。上游未保存质量单列 unknown。正式CSR只保存response query的边；prompt节点属性存在，但prompt→prompt边未观测。本版保留完整节点集合及缓存中的可用响应边，不能称为已捕获完整逐层属性图。层平均边只是 token 依赖代理，不是跨 Transformer 层的 attention rollout。
...
研究主假设直接绑定首错定位：在预固定任务/生成器/来源采样总体中，graph相对mass_matched_uniform的来源宏平均首错midrank percentile差为负。支持性假设为同样来源内AUROC差为正。这些不是无标签拟合或校准定理的结论，只能在冻结自然评价中检验。对高置信、图上自洽的错误允许失败。
...
3. 主硬门槛：同一来源配对bootstrap的graph−mass_matched_uniform首错midrank percentile差，其95%区间上界须<0；支持性门槛为回答内AUROC差的95%区间下界>0。前者不成立就删除“图改善首次定位”的贡献；后者不成立就不声称全token回答内排序改善。no_neighbors删除对照与permuted_weights只作辅助诊断，不能替代graph-vs-uniform裁决。不能在同一test上调参后宣布新结果。
...
标注每行包含 id、响应token_ids、response相对字符offsets、labels:[{start,end}]；token_ids必须与冻结observer响应token完全相同，正长度token与标注span部分重叠即为阳性。缺标注或token身份错配报错；零长度偏移标为不可评价并逐回答报告覆盖，不能当正常token。
...
评价各子集按“来源等权→该子集有效回答等权→该回答有效token等权”重算权重。
首错指标采用从回答开头到每答第一次错误的候选集合，报告含并列midrank的归一排名和倒数排名、首错前报警；均为后验定位。回答内AUROC按来源宏平均，提供固定种子1000次来源bootstrap区间，少于2个可用来源不输出区间。每个子集都保留token/阳性分母。
本版主分数是Gaussian工作模型下的残差二次型；编码nats只作同排序诊断。当前token query的目标条件化不会通过“没有直接输入z_i”消失。

补充近邻文献：[TOHA, ACL 2026](https://aclanthology.org/2026.acl-long.704/) 已用attention图拓扑散度与少量标注进行检测；[Attention Divergence, ACL SRW 2026](https://aclanthology.org/2026.acl-srw.6/) 是相关内部散度路线；[First Hallucination Tokens Are Different from Conditional Ones](https://arxiv.org/abs/2507.20836) 支持将首次与条件延续分别研究。这里只陈述关联工作，不把它们的结果移植为本方法证据。


## 第二轮修订：身份与配对裁决

hidden缓存必须包含标量JSON字符串 `hidden_schema`，至少提供非空字符串字段
observer_model、observer_revision、tokenizer、tokenizer_revision、layer、state_location。
revision须为实际冻结版本/权重标识；state_location明确pre/post norm和层位置。
程序对完整schema规范JSON计算hash，跨所有来源/分区校验一致并随模型保存；该声明仍需真实采集器保证内容对应，不能靠伪造metadata证明来源。
原始hidden保留float32；固定投影和线性求解使用float64。`--max-attribute-mib`默认1024，限制当前加载节点属性总量，超过就停止；它不是进程总RSS上限，单文件解压、CSR和拟合中间数组还需额外内存。正式大规模运行须先冻结来源清单并测峰值RSS。
评价输出graph-uniform同来源的配对差值与1000次bootstrap区间；首错归一排名越低越好、AUROC越高越好。少于2个eligible来源只报告点差，无区间，不能据此通过门槛。
