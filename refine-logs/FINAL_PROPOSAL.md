> 当前入口已更新为 docs/GROUNDED_GRAPH_RESTORATION_V2_20260913.md 与 docs/GROUNDED_GRAPH_RESTORATION_V2_RUN_20260913.md。A2-v1完整负结果见 docs/GROUNDED_GRAPH_V1_RESULTS_20260913.md；本文件以下为保留的历史方案，不能将未验证设想当现行实现或结果。
...
当前完整主线未实证收敛。方法结构与边界见 [CURRENT_METHOD_20260913.md](../docs/CURRENT_METHOD_20260913.md)。全量population17790/17790完成、failed0；typed full仅2对照，真实train native90forward仅raw影响、强证书0。SourceRel-Mini已实现数据/特征/训练CLI，883来源、7064query，完整长度预检通过；两来源sanity完成，全量编码/训练推进中。TFIDF source-validation top1=80.60%，不是自然归属准确率。自然迁移与路由/连续span仍待验证。
...
最新方案：[事实范围与原生图修订](fact_scope_graph_revision_20260913.md)；结果：[surface owner负结果](../docs/SURFACE_OWNER_V1_RESULTS_20260913.md)。以下旧“当前”按历史阅读。
...
来源核查使用两次互不传递前次答案/answerability的提示，每个条件从问题原文映射到来源引用span。E/C需两个精确答案一致且条件表通过；答案在被引用证据内解析唯一occurrence。N另需所有source单位在全文上下文核查后均无未决答案。

最多4个不同Claim。按风险和原文顺序选择；如有当前E、此前C且同全部条件的纠正控制，预留1席，其余最多3个风险Claim。纠正以此前错误答案构造明确错误alternative，不能把正向历史依赖当作错误继承。所有未选和无法对比的Claim留在分母。

N先验证位置/介导与第二模板，再做层带；paired/MLP在N明确不测。输入介导控制必须同角色，匹配长度、embedding范数和pairwise因果可达性。MLP须当前E×M的实际联合sham；MLP范围超出证据组或context项占主导时只报broad_MLP_event_interaction。

原生搜索首版为完整token域二分与非相邻query并组；标点/64-token叶节点用于全文条件核对和图坐标，不声称已学习最优语义边界。所有跨span长边保留。阶段缓存必须绑定完整B/A、候选和全部上游哈希，真实donor向量保留并复验。
...
增加的算子仍来自同一冻结模型，不新增训练头。每风险Claim硬上限320次模型forward，每回答上限1280次（最多4个按固定语义风险/原文顺序选择的Claim）；未纳入者保留语义结果和budget_unresolved。branch B/A各计一次，donor prefix也计一次，同时报告处理token数与时延，不用前缀forward冒充免费。

| 用途 | 最大forward数 | 固定规则 |
|---|---:|---|
| B/D base及初始sham | 8 | 两阶段分别重算B/A，未包含在后续缓存中免费计算 |
| candidate-blind搜索 | 64 | 实际根组、子组、非相邻query并组 |
| 位置/角色验证 | 36 | 最多3组，各自sham/half/两control/keep/repeat |
| 原始输入介导 | 48 | 最多2组，full/half/sham/两control/repeat，每条件4个前向 |
| paired R | 12 | C或纠正条件，同H同移出质量，只改目的地；N不测 |
| E_app×MLP | 24 | C或纠正条件，真实四态、同MLP交互控制、联合sham；N不测 |
| N模板复测 | 40 | 一个预选content位置及对应已通过origin，另一个固定非承诺模板 |
| 层带复核 | 42 | 一个最终组×3层带，每带具体sham及完整验证 |
| 保留 | 46 | 仅预算余量，不为凑数或成功回填增加试验 |
| 保守合计 | 320 | C与N块互斥；实际开销逐前向记录 |
...
每回答1280调用是**机制教师的上限**，不是承诺这个成本适于17790全量主线检测。先测完整自然batch实际成本和覆盖；若计算瓶颈成立，唯一允许的后续学习模块是用实测Δ/Ω为目标的图读出蒸馏，需先证明教师准确/有覆盖。没有可靠教师前不训练一个看起来轻量但目标未成立的GNN。主线部署速度与教师诊断速度分开评价，当前不能提供已收敛全量指令。
...
当前代码：causal_contrast.py与native_audit.py已经实现计算及重放算子；已编码上述A–D自动编排，部署前工程复审中；没有完整自然执行或有效性结果。
...
资源：Qwen只生成有长度界限的JSON/问答；语义调用数随自动Claim数计入，超限弃权而不丢样本。Llama机制教师每风险Claim≤320、每回答≤1280实际前向，比现有8条件全量更贵；先测完整自然batch的真实延迟/显存，再决定全量预算。没有凭空给6–16小时承诺。
...
## Final executable slot-link constraint — 2026-09-13T04:35:23+08:00

Continuous-error and supported-recovery typed edges require C to bind the current answer slot to an exact previous question ID. C receives prior question/answer text without factual labels or native effects. D then scales only the mapped previous answer-slot input embeddings, captures branch-matched selected V, and tests mediation. Whole previous-claim mediation without this slot binding remains a conditional history dependency and cannot certify error continuation. Both prior/current unsupported estimates and all original selectivity/template criteria remain required.

Layer-band budget exhaustion is layer_scope_unresolved, never evidence of distributed causation. Risk and recovery claim denominators are separately deduplicated by claim span. Search records preserve all structural control IDs/matching, unsuccessful forward/token costs, and unsearched tree/union groups.

The frozen raw-input control pool now uses windows matched to each tested origin length (including one-token prior answer slots), same role, embedding norm and pairwise causal reach. These windows are constructed after B native search but before C labels, with hash-first16 per origin. C sees only fixed text; D selects hash-first2 unrelated controls without effect-based backfill. Native proposal/search itself is unchanged. Raw source-unit IDs used for N completeness remain separate from these input controls.
