...
上述二步量命名为 two-step source-through-history residual，不称为纯二阶 interaction：它包含前一段、后一段的一阶端点偏离以及它们的交叉项。当前不增加 hybrid terms，不声称已隔离纯 interaction；通过 d=1 对照检验二步读取是否增加检测信息。
...
主分数：每种表征用自己的参考中位数居中；每个坐标的共享尺度取四种表征各自参考标准差的最大值。共同 active mask 为共享标准差 > 1e-6；所有表征在同一 active 坐标上计算 mean(z²)，方向固定为越大越异常。这取消 MAD 分母为零的放大问题，不使用多级 fallback。参考中始终不变的坐标不参与分数，因此也放弃检测该坐标在测试中才出现的新变化；若整组无 active 坐标，四种分数明确为零，并逐 token 标记 reference_active_features=0，不把它解释成可信正常。标准差对参考污染敏感，作为方法限制记录。补充 kNN 分数：对每个不同参考来源取最近样本距离，再平均最近 k=3 个来源距离。条件中不足 k 个参考来源时明确失败，不跨任务／位置条件回退。
...
接受常数坐标风险并用四表征共享标准差／active mask 修正；新增回归测试防止 0.001 的未见舍入差被放大为 500000 的异常分数。接受 two-step residual 不等于 pure interaction 的解释边界；不为追求名义二阶创新添加额外算子。原始来源／路径×候选×层×通道的目标保持不变。自然性能待证，不能靠审查评分弥补。
