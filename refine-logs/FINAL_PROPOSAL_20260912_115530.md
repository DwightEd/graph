...
更新时间：20260912_115530 UTC。状态：REVISE，以下新读出尚无结果；旧路径残差继续作为冻结基线。
...
6. 风险方向预声明为 1-G_top1/Σ_c G_c；相同定义用于 D。无覆盖、零上下文质量或仅一个有覆盖候选都标记不可判定，绝不当作可信正常。
...
沿用 local_to_remote 的共同可见 key 与当前远近划分，修正并测试特殊 token 排除；窗口 8/16/32，主窗口16。与旧 W1+过去基线比较。只使用过去或 fit 来源确定阈值，固定事件预算与去重间隔。先报告连续分数及边界误报；已有手工自然窗口不能用于声称自动定位率。
...
当前两项均待证。C1 失败即停止把 B 接入主线检测；C2 无增益即撤回图必要性主张。
