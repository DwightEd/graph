...
class AttentionCollector:
    def __init__(self, *, num_layers: int, num_heads: int, num_tokens: int, response_idx: int, floor: float, dtype: torch.dtype) -> None:
    ...
    def consume(self, layer_index: int, attention: torch.Tensor) -> None:
        ...
        self.diagonals[layer_index] = attention.diagonal(dim1=-2, dim2=-1).to(dtype=self.dtype, device="cpu")
    ...
    def finalize(self) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        return (
            ...
            torch.cat(self.column_indices) if self.column_indices else torch.empty(0, dtype=torch.int32),
...
class AttentionExtractor:
    ...
    def run(self) -> None:
        ...
        model = AutoModelForCausalLM.from_pretrained(
            self.config.model_path, torch_dtype=dtype, attn_implementation="eager"
        ).to(self.config.device).eval()
        samples = load_ragtruth_samples(
            self.config.dataset_path, split=self.config.split,
            generator_model=self.config.generator_model, task_type=self.config.task_type,
        )
        ...
        for sample in tqdm(samples, desc=f"RAGTruth {self.config.split}"):
            token_ids, response_idx = tokenize_ragtruth_sample(tokenizer, prompt=sample.prompt, response=sample.response)
            ...
            def collect_attention(_: torch.nn.Module, __: tuple[Any, ...], output: tuple[torch.Tensor, torch.Tensor], layer_index: int) -> tuple[torch.Tensor, None]:
                nonlocal collector, head_count
                attention = output[1]
                if attention.ndim != 4 or attention.shape[0] != 1:
                    raise ValueError("Llama decoder layer must return [batch, heads, tokens, tokens] attention")
                layer_attention = attention[0]
                if collector is None:
                    head_count = int(layer_attention.shape[0])
                    collector = AttentionCollector(num_layers=layer_count, num_heads=head_count, num_tokens=token_ids.numel(), response_idx=response_idx, floor=self.config.floor, dtype=dtype)
                collector.consume(layer_index, layer_attention)
                compact = list(output)
                compact[1] = None
                return tuple(compact)

            hooks = [layer.self_attn.register_forward_hook(lambda module, args, output, index=index: collect_attention(module, args, output, index)) for index, layer in enumerate(model.model.layers)]
            try:
                input_ids = token_ids.unsqueeze(0).to(self.config.device)
                with torch.no_grad():
                    model.model(input_ids=input_ids, attention_mask=torch.ones_like(input_ids), return_dict=True, use_cache=False, output_attentions=True)
            finally:
                for hook in hooks:
                    hook.remove()
            if collector is None:
            ...
            artifact = AttentionSample(sample.response_id, sample.source_id, response_idx, token_ids, diagonal, row_ptr, columns, values, self.config.floor)
            filename = f"{sample.response_id}.pt"
            save_attention_sample(artifact, output_dir / filename)
            index_rows.append({"sample_id": sample.response_id, "source_id": sample.source_id, "path": filename})
        manifest = {
            "schema": "attention-response-csr-v1", "observer_model": self.config.model_path,
            "tokenizer": getattr(tokenizer, "name_or_path", self.config.model_path),
            "num_layers": layer_count, "num_heads": head_count, "dtype": self.config.dtype,
            "floor": self.config.floor, "channel_order": "layer,head", "split": self.config.split,
            "generator_model": self.config.generator_model, "task_type": self.config.task_type,
            "input_policy": "full_context_no_truncation", "count": len(index_rows),
        }
