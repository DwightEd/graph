...
class SampleBehaviorVisualizer:
    ...
    def __init__(self, split_root, *, labels_path=None, device="cpu", verify_hashes=False):
        self.dataset = ResearchDataset(split_root, device=device, verify_hashes=verify_hashes)
        labels_path = Path(labels_path) if labels_path is not None else self.dataset.root / "labels.jsonl"
        self.labels = LabelStore(labels_path)
        missing = set(self.dataset.sample_ids).difference(self.labels.rows)
        if missing:
            raise ValueError(
                f"labels are missing {len(missing)} dataset sample IDs; first: {sorted(missing)[:3]}"
            )
    ...
    def analyze(self, sample_id) -> dict:
        ...
        response_labels = self.labels.response_labels(sample, attention=attention)
        return {
            "sample_id": sample_id,
