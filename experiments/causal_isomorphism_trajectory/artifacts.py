...
def load_reference(path):
    ...
    for variant in VARIANTS:
        values = np.asarray(
            reference[f"calibration_energy_{variant}"],
            dtype=np.float64,
        )
        if (
            values.ndim != 1
            ...
            raise ValueError(
                f"CITG {variant} calibration energy is invalid"
            )
...
def load_score_artifact(path):
    ...
    if row_count < 1 or any(
        len(artifact[name]) != row_count for name in row_fields
    ):
    ...
    if bool((~np.isfinite(rewired[valid])).any()) or bool(
        (~np.isfinite(gap[valid])).any()
    ):
    ...
    validate_source_audit(
        reserved_source_ids=np.concatenate(
            (
                artifact["fit_group_id"],
                artifact["calibration_group_id"],
            )
        ),
...
def verify_score_provenance(artifact):
    ...
    reference_file = FrozenFile.capture(
        scalar_text(artifact, "reference_path")
    )
    if reference_file.sha256 != sha256_text(
        artifact, "reference_sha256"
    ):
        raise ValueError(
            "CITG score reference digest differs from its artifact"
        )
    ...
    for field in ("fit_group_id", "calibration_group_id"):
        if not np.array_equal(
            ...
            raise ValueError(
                f"CITG score {field} differs from its reference"
            )
    return reference
