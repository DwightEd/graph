...
实现更新：`constraint_flow.py` 已实现共同有效前缀的精确配对差分和固定候选读出；
`constraint_flow_run` 提供可控实体绑定／语义控制、冻结路径及限定 value 通道的功能确认。
当前范围、审稿意见、输入协议和一键运行见 [ICLR_REVIEW.md](ICLR_REVIEW.md)。
四种失败分类、后段 2×2、真实 RAGTruth 普遍性和最终检测仍是研究计划。
...
**处理决定。** 撤下 README 中的全量原生采集推荐；保留代码、已有 trace 和报告供核查，不为 PCA 继续新增采集、不围绕这批 test 调参数。
已有 trace 只能承担其实际记录的自然轨迹观察。约束差分和限定通道确认现已实现，但尚无真实 RAGTruth 结果；
必须先获得形成与后续使用的实证，再定义对应检测量，当前不承诺下一方案的 AUROC。
...
配对引擎保留整个共同有效前缀的逐 head 净写入和状态，固定 query 的所有来源记账，以及登记根位置的入边；
不需要先通过“关键节点阈值”。它没有保存所有 query×source 的完整向量，也没有对自然回答全部 token 定义事实候选。
候选 hub 只是少量详细确认的定位对象。只有在探索集确有重复形态后，才冻结事件识别规则并在独立来源检验。
...
| `constraint_flow.py`：`ConstraintPair`、`ConstraintObserver` | 对齐输入、真实 AV/模块差分、同一判断的输出账、冻结 value-path 确认 | 已实现；保留所有 head，功能范围见 ICLR_REVIEW.md |
| `constraint_flow_run.py` | 核验输入契约、事实／重命名／无关／措辞四比较、续跑、预算、来源报告 | 已实现；支持内置可控案例和外部核验 JSONL |
| `constraint_flow_plot.py` | 逐 head、同一目标的阶段账、冻结路径与总体对照图 | 已实现 |
| `detection_metrics.py` | 完整 token 评价与来源级区间 | 已有；最终评分算法尚未确定 |

已实现的核心接口：

```python
pair = ConstraintPair(token_ids, response_start, negative_id, positive_id)
trace = capture_constraint_pair(model, pair, query_chunk=8)
paths, roles = select_constraint_paths(model, trace)
# 编排层先保存 trace 和 paths；对照沿用路径，不另行筛选。
confirmation = confirm_paths(model, trace, paths, query_chunk=8)
```

`ConstraintPair` 只负责对齐输入和固定判断，语义核验声明由输入编排层提供。
`ConstraintObserver` 不读幻觉标签、不训练分类器、不决定绘图颜色。配对载荷、分解公式和路径查询聚在同一模块。

旧 `routing_probe`、`native_patterns` 仅作对照；旧 `route_model` 的代理 provenance 不作为新模型的事实真值。
结构入口保留作对照；配对核心已实现但真实语义效应尚未得到验证，旧复现代码不承担这一角色。
...
- 基础配对需要两个条件的前向。当前实现 batch=2、同一模型、分块计算；四个比较实际重算 baseline，共 8 个条件前向。
  只有缓存基线所有必要量才可能把 K 个变体降为 1+K，当前不作此宣传。固定路径确认成本另计。
...
- 固定候选方向乘最终 norm 的 gamma 在运行前已知；当前实现先记此投影，最后再乘平均 inverse RMS，单列 norm 差分项。
  因此配对读出无需第三次测量重放。采集时间、峰值显存、磁盘和额外确认调用次数均保存。
...
随后 `constraint_flow_run` 实现共同有效前缀的对称 AV 差分、原生模块与最终 norm 账，以及固定预算的两跳
value 通道确认。候选从 q 的内容响应定位，不再以 WAAD 峰为必要条件；无关／措辞对照沿用事实条件路径。
内置可控绑定输入可以直接跑，外部 JSONL 仍需语义核验。数值测试使用 tiny HF Llama，不能作为 8B 的机制证据。

尚未完成：自然样本的无损完整图谱、真实配对数据集、完整 Q/K 与冗余中介、后段 2×2、独立来源机制复现，
以及最终检测评分。现有 `discover` 或新先导入口运行成功都不自动完成这些研究工作。
