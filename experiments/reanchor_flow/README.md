...
**当前主线与审稿意见见 [ICLR_REVIEW.md](ICLR_REVIEW.md)，包含完整一键指令。**
新增 `constraint_flow_run` 实现有效条件配对、逐 head 内容／路由精确差分、原生 MLP／RMSNorm 记账，
并对冻结的少量两跳 value 路径确认输出作用。候选不再必须有 WAAD 峰；控制条件沿用事实条件的同一组路径。
先用内置的小型实体绑定实验核查功能，再迁移到经过核验的真实前缀；这不是新的幻觉检测器，没有 AUROC。

[原始 attention 与原生载体审计 v2](ATTENTION_RHYTHM.md) 保留为自然轨迹的结构对照，已有输出仍然可用。
两个入口都有 tqdm、续跑和不加载模型权重的离线分析。不要再运行下面的 PCA 距离作为主检测器。
