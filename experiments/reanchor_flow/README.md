...
**当前可运行的审计入口是 [原始 attention 与原生载体审计 v2](ATTENTION_RHYTHM.md)。**
该文档给出 train/test、全部任务和完整 response 的一键指令。它输出逐 head 原图/趋势、同一载体的合法跨层关系，
并在少量无标签展示样本中记录真实两跳消息、完整残差/MLP 和所有 head 的读出；有 tqdm、续跑和不加载模型的离线分析。
结构与原生运算记录不等于事实因果验证，本入口没有新的机制检测 AUROC。不要再运行下面的 PCA 距离作为主检测器。
