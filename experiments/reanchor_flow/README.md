...
输出逐 head 读出及固定检测对照。它是图模型的已实现组件；逐来源单位、同目标全路径贡献和联合节点结构报告尚未完成，
具体定义与实现边界见方法文档 §6.1–6.3。`material_deficit` 是待检验假设，不代表已经发现的机制。
不要求 WAAD 峰、不截断负贡献、不训练 GNN，不对每条路径消融。尚无新的真实 8B 机制或性能结论。

原始采集仍使用 [正常／幻觉结构审计 v3](ATTENTION_AUDIT_V3.md)，完整命令、覆盖表和 notebook 使用见该文档。
运行 `attention_audit_run`：排除特殊 token 的主要统计；明确 N/H 与正负回答；位置插值和起点对照；
全部跨层 head 对的进入/复用四状态与目标两跳统计；一次前向保存实际 Q/K、完整 history、原生向量和读出。
所有样本有标红全文，可离线重建任意 head 的完整来源图。开始前检查全量输入与标签并打印存储估算，之后显示样本/层/保存/分析进度。
`audit train/QA` 是读取缓存的 CPU 统计；现在还显示 history 复用、两跳 reader 层、四状态配对、保存及样本耗时。
32×32 heads 对应 507,904 个合法跨层 head 对。穷举这些配对不等于发现完整多跳消息机制。

旧 [v2](ATTENTION_RHYTHM.md) 结果不删除，旧入口仅用于复现。v2 的 36 条展示路径全部选中 BOS，
不能作为相关事实中继的实例；v3 不再沿用峰交集和最大任意来源的候选规则。
[约束配对工具与审稿意见](ICLR_REVIEW.md) 保留作功能验证参考，不替代真实数据的正负结构比较。
这些工具均不是已验证的幻觉检测器。新来源分解有独立的固定评分与评估入口；不要再为下面的 PCA 距离路线全量采集。

当前研究主线是：**被后续生成复用的中间状态怎样形成，以及它对输入约束的响应是否实际影响后续输出。**
项目讨论与真实结果总结、假设、论文依据、配对约束差分算法、hub 验证、检测迁移和实现顺序统一记录在
[MECHANISM_AUDIT.md](MECHANISM_AUDIT.md)。配对核心和限定 value 通道的确认已经实现；
真实数据上的内容特异性、冗余／QK 通路、四类失败与最终检测评分仍未验证。
...
能逐位置核查的形成／复用过程；新配对入口的数值测试不等于机制实证，不能承诺其检测效果。
