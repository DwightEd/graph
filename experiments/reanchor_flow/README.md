...
**当前主线见 [消息来源、DAG 与固定检测评估](MESSAGE_LINEAGE.md)。**
已完成的 v3 缓存先运行 `message_lineage_run --completed-only`：材料向量通过真实 value、残差与 MLP 的条件分解传播，
得到逐 head 的直接／中继读出和固定无监督分数，直接输出同 token 集合的 AUROC/AP 与位置、logprob、直接读出对照。
不要求 WAAD 峰、不截断负贡献、不训练 GNN，不对每条路径消融。这个方法尚未有新的真实 8B 性能结论。
