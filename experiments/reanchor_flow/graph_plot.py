...
def draw_top_graph(axis, result: dict, label: np.ndarray) -> None:
    ...
    revisit = np.asarray(result["revisit_peak"], dtype=bool)
    anchor = np.asarray(result["anchor_peak"], dtype=bool)
    future = np.asarray(result["future_influence"], dtype=float)
    events, _ = edge.shape

    target_candidates = set(np.flatnonzero(revisit).tolist()) | set(
        np.flatnonzero(anchor).tolist()
    )
    if len(target_candidates) < 6:
        score = np.nan_to_num(robust_z(result["revisit_delta"]), nan=-np.inf)
    ...
    for event in target_candidates:
        for source in np.argsort(-edge[event])[:3]:
            ...
            if weight > 0:
                candidates.append(
                    (weight, int(source), response_start + int(event))
                )
    candidates = sorted(candidates, reverse=True)[:30]
    nodes = sorted(
        {node for _, source, target in candidates for node in (source, target)}
    )
    ...
    other_prompt_nodes = [
        node for node in prompt_nodes if node not in evidence_nodes
    ]
    ...
    def response_sizes(items):
        return [
            ...
            * max(
                0.0,
                min(4.0, future_z[node - response_start] + 1.0),
            )
            for node in items
        ]
    ...
    ordinary = [
        node
        for node in response_nodes
        if not revisit[node - response_start]
        and not anchor[node - response_start]
    ]
    if ordinary:
        axis.scatter(
    ...
    revisit_nodes = [
        node for node in response_nodes if revisit[node - response_start]
    ]
    if revisit_nodes:
        axis.scatter(
            [position[node][0] for node in revisit_nodes],
            [position[node][1] for node in revisit_nodes],
            marker="D",
            s=response_sizes(revisit_nodes),
            label="prompt revisit",
        )
    anchor_nodes = [
        node for node in response_nodes if anchor[node - response_start]
    ]
    if anchor_nodes:
        axis.scatter(
    ...
    hallucinated = [
        node
        for node in response_nodes
        if 0 <= node - response_start < len(label)
        and label[node - response_start]
    ]
    if hallucinated:
        axis.scatter(
    ...
    axis.set_xticks([])
    axis.set_yticks([])
    axis.set_title("D  Top functional routes", loc="left", fontweight="semibold")
