...
def save_sample_figure(
    ...
    *,
    ...
    try:
        ...
        axes[0, 0].set_title(
            "A  Source → predicted-token route map",
            loc="left",
            fontweight="semibold",
        )
        ...
        axes[0, 1].scatter(
            np.flatnonzero(revisit),
            revisit_z[revisit],
            marker="D",
            label="revisit peak",
        )
        axes[0, 1].scatter(
            np.flatnonzero(anchor),
            future_z[anchor],
            marker="^",
            label="anchor peak",
        )
        ...
        ranking = np.argsort(-np.nanmax(flattened, axis=1))[
            : min(64, len(flattened))
        ]
