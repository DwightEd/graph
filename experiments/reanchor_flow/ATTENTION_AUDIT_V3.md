...
**采集完成后的新核心入口是 [message_dag.run](message_dag/README.md)：来源单位传播、同目标全路径贡献和 N/H 结构比较。
它直接复用本页描述的缓存，不需要先完成旧版 `audit train/QA` 的全部 head-pair 统计。**

**当前研究主线与实现边界见 [DAG 消息路由建模与幻觉机制发现](MESSAGE_LINEAGE.md)。已有完整状态可用 `message_lineage_run` 做材料向量的 value／残差／MLP 分解和固定检测对照；完整同目标多跳贡献图尚未实现。无需为这一组件重新采集，支持中断后的已完成子集。**
...
`audit train/QA` 表示缓存上的 CPU 统计，外层进度单位为 source；内部显示样本编号、history／未来复用层、
两跳 reader 层、四状态 head 配对、保存与样本累计耗时。32 层×32 heads 有 507,904 个严格跨层 head 对，
全配对矩阵和 history 解压会耗时；此阶段不运行 LLM，也不传播完整多跳消息向量。
进度改动只增加过程可见性，不减少原统计计算量；已启动的 Python 进程不会因 git pull 自动更新代码。
