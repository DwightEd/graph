...
def test_response_relay_preserves_prompt_lineage_instead_of_marking_it_detached():
    ...
    assert final[LINEAGE_INDEX["detached_one_hop"]] == 0.0


def test_response_relay_of_local_token_is_detached_and_mass_is_conserved():
    diagonal = torch.zeros((2, 2, 1))
    diagonal[0, 0, 0] = 1.0
    routing = routing_state(
        layers=[1],
    ...
    assert trace.state[1, -1, LINEAGE_INDEX["detached_one_hop"]] == 1.0
