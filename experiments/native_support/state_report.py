...
def state_table(rows, baseline):
    fields = (baseline, "route_mean", "joint_state", "reset_probability", "expected_run_length",
              "reset_contribution", "continuation_contribution", "state_route_sd")
    headers = ("当前路由", "普通均值", "联合状态", "切换概率", "期望段长", "新段贡献", "延续贡献", "状态标准差")
...
def write_state_report(path, rows, summary, evaluation, comparisons):
    content = [
        ...
        '输出是功能路由状态的后验均值，不是幻觉概率；统计切换不等于语义重锚。</p>',
    ...
    for identity in dict.fromkeys(row["response_id"] for row in rows):
        ...
        content.extend((f'<h2>{escape(identity)}</h2><p>蓝：联合状态；橙：普通均值。</p>',
                        curves(selected, ("joint_state", "route_mean")), state_table(selected, summary["raw_baseline"])))
