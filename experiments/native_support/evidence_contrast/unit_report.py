...
def load_records(output, settings, methods=METHODS):
    records, predictions = [], []
...
def unit_metrics(rows, token_methods=TOKEN_METHODS):
...
def localization_metrics(records, predictions, methods=METHODS):
    ...
    status = "evaluated" if measured[methods[0]]["mixed_units"] else "unavailable_no_mixed_units"
...
def plot_comparison(output, evaluation, token_methods=TOKEN_METHODS):
    import matplotlib
    matplotlib.use("Agg")
    ...
    figure.suptitle("Same tokens and text units | exploratory evaluation")
    figure.tight_layout()
...
def evaluate_units(output, settings, methods=METHODS, token_methods=TOKEN_METHODS,
    ...
    write_json(output / "unit_evaluation.json", unit_metrics(rows, token_methods))
    write_json(output / "within_unit.json", localization_metrics(records, predictions, methods))
    write_json(output / "annotation_alignment.json", annotation_alignment(rows))
    write_json(output / "unit_budget.json", budget)
    plot_comparison(output, result, token_methods)
    return result
