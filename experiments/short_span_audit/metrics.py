...
from .comparisons import compare_matched_methods, compare_methods, complete_pairs
...
def _common_blocks(blocks, methods):
    ...
    for block in blocks:
        ...
        scores = {method: np.where(finite, block["scores"][method], np.nan) for method in methods}
...
def _alarm_blocks(blocks, method, threshold):
    if threshold is None:
        return blocks
    return [
        dict(block, alarms={method: block["scores"][method].astype(np.float64) >= threshold})
        for block in blocks
    ]
...
def _span_tables(blocks, method, metadata):
    ...
    for block in blocks:
        ...
        gold = merge_token_spans(block["gold"])
        for index, (start, end) in enumerate(gold):
            next_start = int(gold[index + 1, 0]) if index + 1 < len(gold) else block["tokens"]
            spans.append(
                dict(
                    metadata,
                    **identity,
                    ...
                    **deadline_measurements(block, method, start, end),
                    **recovery_measurement(block, method, end, next_start),
                )
            )
        ...
        for pair in block["pairs"]:
            ...
            for side in ("error", "normal"):
                ...
                paired.append(
                    dict(pair, **metadata, **identity, side=side, normal_history=history, **row)
                )
    return spans, paired
...
def _span_summary(rows):
    ...
    summary.update(
        ...
        complete_miss=summary["completely_observed_missed"],
        ...
        span_weighting="one_per_span",
        delay_population="detected_only",
    )
    return summary
...
def _paired_ranking(rows):
    ...
    pairs, selected = complete_pairs(rows)
    complete = list(selected.values())
    ...
    wins = (error > normal) + 0.5 * (error == normal)
    return {
        "pairs": len(pairs),
...
def _paired_confusion(rows):
    _, complete = complete_pairs(rows)
    count = len(complete)
    tp = sum(pair["error"]["observed_any_alarm"] for pair in complete.values())
    fp = sum(pair["normal"]["observed_any_alarm"] for pair in complete.values())
    return {
        "tp": tp,
        "fn": count - tp,
        "fp": fp,
        ...
        "recall_minus_fpr": (tp - fp) / count if count else None,
        "population": "complete_pairs_only",
    }
...
def _paired_summary(rows):
    ...
    for history in ("all", "clean_history", "recovery"):
        selected = (
            rows if history == "all" else [row for row in rows if row["normal_history"] == history]
        )
        result[history] = {
            "ranking": _paired_ranking(selected),
            "confusion": _paired_confusion(selected),
        }
        for side in ("error", "normal"):
...
def _setting(blocks, method, group, threshold, budget, setting):
    ...
    answers = answer_measurements(selected, method, metadata)
    result = dict(metadata, threshold=threshold, **_normal_alarm_summary(selected, method))
    result["answer_alarms"] = answer_summary(answers)
    result["threshold_source"] = (
        "saved_frozen_alarms" if threshold is None else "test_normal_labels_descriptive_only"
    )
    ...
    for name, limits in LENGTH_BINS.items():
        ...
        history = {
            state: _span_summary(
                [row for row in error_rows if row["recent_error_history"] == state]
            )
            for state in ("clean_history", "recovery")
        }
        result["bins"][name] = {
            "error": dict(_span_summary(error_rows), **lifecycle_summary(error_rows)),
            "error_history": history,
            "matched": _paired_summary(pair_rows),
        }
    return result, spans, paired, answers


def _evaluate_group(blocks, methods, group, fpr_budgets, bootstrap, seed):
    results, spans, paired, answers = {}, [], [], []
    for method in methods:
        frozen, error_rows, pair_rows, answer_rows = _setting(
            blocks, method, group, None, None, "frozen"
        )
        spans.extend(error_rows)
        paired.extend(pair_rows)
        answers.extend(answer_rows)
        curve = []
        normal_scores = _normal_scores(blocks, method)
        for budget in fpr_budgets:
            threshold = normal_fpr_threshold(normal_scores, budget)
            measurement, error_rows, pair_rows, answer_rows = _setting(
    ...
    comparisons = compare_methods(spans, methods, bootstrap, seed)
    comparisons.extend(compare_matched_methods(paired, methods, bootstrap, seed))
...
def evaluate(blocks, methods, fpr_budgets=(0.01, 0.03, 0.05), bootstrap=200, seed=17):
    ...
    report = {"groups": {}, "span_rows": [], "paired_rows": [], "answer_rows": []}
    for group, selected in grouped.items():
        result, spans, paired, answers = _evaluate_group(
            selected, methods, group, fpr_budgets, bootstrap, seed
        )
        report["groups"][group] = result
        report["span_rows"].extend(spans)
        report["paired_rows"].extend(paired)
        report["answer_rows"].extend(answers)
    return report
