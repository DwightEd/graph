...
OUT=${OUT:-experiments/spectral_feasibility/outputs/causal_dual_spectrum_v1}
DEVICE=${DEVICE:-cuda}
...
echo "device=$DEVICE"
echo "top_k=${TOP_K:-5}"
echo "prompt_sketch_dim=${PROMPT_SKETCH_DIM:-4}"
...
echo "[1/3] fit label-free causal dual spectrum on train"
python -m experiments.spectral_feasibility.main fit \
  --train-split "$ROOT/train" \
  --output "$OUT/reference.npz" \
  --device "$DEVICE" \
  --top-k "${TOP_K:-5}" \
  --prompt-sketch-dim "${PROMPT_SKETCH_DIM:-4}" \
  --prompt-sketch-seed "${PROMPT_SKETCH_SEED:-20260814}" \
