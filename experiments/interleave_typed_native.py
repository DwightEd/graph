"""Reuse the audited exclusive pause/restore transaction for typed source-path measurement."""
...
def run(args):
    ...
    with (REANCHOR / "runs/relation_interleave_20260913.lock").open("a") as scheduler:
        ...
        with (args.output / ".phase.lock").open("a") as phase:
            ...
            command = [str(PYTHON), "-u", "-m", "next_iteration.typed_native_runner",
                "--preflight", prepared["preflight"], "--output", str(args.output),
                "--observer-model", prepared["observer_model"], "--max-sources", str(prepared["selection"]["cap_sources"]),
                "--inherited-lock-fd", str(phase.fileno())]
            subprocess.run(command + ["--stage", "prepare"], cwd=GRAPH, env=env,
                           pass_fds=(phase.fileno(),), check=True)
            if prepared["protocol"]["schema"] != "typed-native-query-layer@1":
                raise ValueError("typed native protocol differs")
            ...
            try:
                ...
                record = {"status": "prepared", "previous_progress": before, "started_unix": time.time(),
                    ...
                    "scope": "typed_source_path_native_measurement_no_label_evaluation"}
