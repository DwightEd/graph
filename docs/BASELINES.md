# Historical factorial and attention-audit baselines
...
This historical baseline represents each factual commitment event as a fixed-role,
...
The baseline entry point is `python -m control_graph.cli`:
...
python -m control_graph.cli evaluate \
...
```bash
bash scripts/run_factorial_baseline.sh
```
...
- `control_graph/cli.py`: baseline argument parsing and dispatch only.
...
- `docs/FACTORIAL_METHOD.md`: estimands, claims, confounds, and experiment gates.
